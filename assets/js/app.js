...
function renderLevers(){
 ...
 $("#leverKpis").innerHTML=kpi("Facturación",euro(rev),"objetivo 2.400 €")+kpi("Ocupación",percent(occ),"48 h semanales")+kpi("Pendiente",euro(pending),"recuperable")+kpi("Confirmadas",confirmed+"/"+state.appointments.length,"calidad agenda");
 renderSignalMap(rev,occ,pending,confirmed);
 renderCapacityMap();
 renderControlDecision(rev,occ,pending);
 drawBars("#breakEven",[["Ingresos",rev/24],["Equilibrio",1450/24]]); if($("#breakDecision"))$("#breakDecision").innerHTML=`<div class="decision ${rev<1450?"bad":rev<1900?"warn":"ok"}">${rev<1450?"No ampliar costes; llenar agenda.":"Costes cubiertos; optimizar margen."}</div>`;
 ...
 chart("#newRecurring",[["Nuevos",380,"warn"],["Recurrentes",620,"ok"]]); chart("#cashForecast",[["S1",420,"ok"],["S2",510,"ok"],["S3",260,"warn"],["S4",180,"bad"],["S5",390,"warn"]]);
}
function renderSignalMap(rev,occ,pending,confirmed){let el=$("#signalMap");if(!el)return;let items=[
 {t:"Caja",v:euro(rev),n:rev>=1900?"Ritmo sano":rev>=1450?"Cerca del mínimo":"Bajo objetivo",c:rev>=1900?"ok":rev>=1450?"warn":"bad"},
 {t:"Agenda",v:percent(occ),n:occ>=75?"Alta ocupación":occ>=55?"Rellenar huecos":"Activar demanda",c:occ>=75?"ok":occ>=55?"warn":"bad"},
 {t:"Cobros",v:euro(pending),n:pending>180?"Enviar avisos hoy":pending>0?"Seguimiento":"Sin bloqueo",c:pending>180?"bad":pending>0?"warn":"ok"},
 {t:"Citas",v:String(confirmed),n:"confirmadas",c:"info"}
];el.innerHTML=items.map(x=>`<article class="signal-card ${x.c}"><h3><span class="signal-light"></span>${x.t}</h3><strong>${x.v}</strong><p>${x.n}</p></article>`).join("")}
function renderCapacityMap(){let el=$("#capacityMap");if(!el)return;let days=["L","M","X","J","V","S","D"],hours=["10","11","12","13","16","17","18"],levels=[[1,2,3,1,2,0,0],[2,3,4,1,2,1,0],[1,2,3,0,4,1,0],[0,1,2,2,3,2,0],[2,4,2,1,2,2,0],[3,2,0,3,4,1,0],[1,1,2,2,3,0,0]];el.innerHTML=days.map((d,i)=>`<div class="map-col"><div class="map-head">${d}</div>${hours.map((h,r)=>`<div class="map-cell l${levels[r][i]}">${h}</div>`).join("")}</div>`).join("")}
function renderControlDecision(rev,occ,pending){let el=$("#controlDecision");if(!el)return;let cls=pending>180?"bad":occ<55?"warn":"ok",title=pending>180?"Cobrar primero":occ<55?"Rellenar huecos":"Mantener ritmo",text=pending>180?"Prioridad: WhatsApp + email a señales pendientes y vencidas.":occ<55?"Prioridad: activar clientes recurrentes y abrir huecos visibles.":"Agenda y caja en rango; subir ticket en servicios de alta demanda.";el.innerHTML=`<div class="decision control ${cls}"><h3>${title}</h3><p>${text}</p></div>`}
...
function bindOCR(){
 if(!$("#ocrFile"))return;let file=null;
 $("#ocrFile").onchange=e=>{file=e.target.files[0]; if(!file)return; let url=URL.createObjectURL(file); let img=$("#ocrPreview"); img.src=url; img.style.display="block"; $("#ocrStatus").textContent="Imagen cargada. Pulsa Extraer texto.";};
 $("#sampleText").onclick=()=>{$("#ocrText").value="Laura Medina 25/05/2026 10:00 Alicia Tatuaje 180min +34600111222 floral antebrazo 420€\nCarlos Vela 25/05/2026 14:30 Diego Diseño 90min +34600333444 blackwork pecho 160€"; parseOCR()};
 $("#runOCR").onclick=async()=>{if(!file){toast("Sube o fotografía una hoja");return}$("#ocrStatus").textContent="Intentando OCR local del navegador...";
  try{
   if("TextDetector" in window){let det=new TextDetector();let bmp=await createImageBitmap(file);let res=await det.detect(bmp);$("#ocrText").value=res.map(x=>x.rawValue||x.text||"").join("\n");$("#ocrStatus").textContent="Texto extraído con OCR local.";}
   else{$("#ocrStatus").textContent="Este navegador no ofrece OCR local. El módulo queda preparado para API OCR; pega o corrige el texto detectado y convierte.";}
  }catch(e){$("#ocrStatus").textContent="No se pudo extraer OCR local. Pega/corrige el texto y convierte."}
 };
 $("#parseOCR").onclick=parseOCR; $("#importOCR").onclick=importOCR; $("#ocrText").oninput=()=>{if($("#ocrText").value.trim())parseOCR(false)};
}
...
function init(){initBase();bindAgenda();bindPayments();bindTemplates();bindClients();bindOCR();renderIndex();renderLevers()}
